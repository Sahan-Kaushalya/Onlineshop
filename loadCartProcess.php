...
`stock`.`product_id`=`product`.`id` INNER JOIN `color` ON `product`.`color_color_id`=`color`.`color_id` INNER JOIN
`size` ON `product`.`size_size_id`=`size`.`size_id` WHERE `cart`.`user_id`='" . $user["id"] . "'");
